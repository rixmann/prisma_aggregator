...
start_test(Aggregator, Test) ->
    case Test of
	"1" -> gen_server:call(?MODULE, {run_test, {hohes_aufkommen, 0}})
    end.
...
%%--------------------------------------------------------------------
handle_call({run_test, {Test, Params}}, _From, State) ->
    case Test of 
	hohes_aufkommen -> 
	    Subs = [create_json_subscription("http://127.0.0.1:8000/index.yaws", 
					     jlib:jid_to_string(get_sender()), 
					     "ATOM", 
					     "test_hohes_1-" ++ integer_to_list(I)) 
		    || I <- lists:seq(Params, Params + 1000)],
	    mod_prisma_aggregator_tester:send_iq(get_sender(), 
						 jlib:string_to_jid(Aggregator),
						 "subscribeBulk",
						 json_eep:term_to_json(Subs)),
	    Timer = timer:apply_after(120000, gen_server, call, [?MODULE, {run_test, {hohes_aufkommen, Params + 1000}}]),
	    ?INFO_MSG("Test: Hohes Aufkommen, neue Nachrichten werden verschickt ~p~nTimer: ~p", [Params + 1000, Timer]),
	    {reply, ok, State}
    end.
